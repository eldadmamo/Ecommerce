...
const Card = () => {
    ...
    const applyCoupon = () => {
        if (coupon === 'FOURAT') { 
            if((price + shipping_fee) >= 200 && (price + shipping_fee) < 5000) {
                ...
                toast.error("Coupon not applied, Price < 200!");
            }
        } else {
            setDiscount(0);
            setIsCouponValid(false);
    ...
    return (
        <div>
            <Header/>
            <section className='bg-[url("http://localhost:5173/images/banner/shop2.png")] h-[220px] mt-6 bg-cover bg-no-repeat relative bg-left'>
                                       ...
                                       <div className='flex sm:w-full gap-2 w-7/12'>
                                          <div className='flex gap-2 justify-start items-center'>
                                            ...
                                            <div className='pr-4 text-slate-600'>
                                                ...
                                                <span className=''>{pt.productInfo.brand}</span>
                                            </div>
                                          </div>
                                      </div>
...

                                 {
                                    ...
                                    <div className='flex flex-col gap-3'>
                                        <div className='bg-white p-4'>
                                            <h2 className='text-md text-green-500 font-semibold'>Out of Stock {outofstock_products}</h2>
                                        </div>
                                       ...
                                       <div className='flex sm:w-full gap-2 w-7/12'>
                                          <div className='flex gap-2 justify-start items-center'>
                                            ...
                                            <div className='pr-4 text-slate-600'>
                                                ...
                                                <span className=''>Brand: {p.products[0].brand} </span>
                                            </div>
                                          </div>
                                      </div>
                                    ...
                                    <div className='flex justify-between w-5/12 sm:w-full sm:mt-3'>
                                        ...
                                        <div className='flex gap-2 flex-col'>
                                            <div className='flex bg-slate-200 h-[30px] justify-center items-center text-xl'>
                                                ...
                                                <div onClick={()=> inc(p.quantity, p.productInfo.stock, p._id)} className='px-3 cursor-pointer'>-</div>
                                            </div>
                        ...
                        <div className='pl-3 md-lg:pl-0 md-lg:mt-5'>
                            {
                                ...
                                <div className='bg-white p-3 text-slate-600 flex flex-col gap-3'>
                                    ...
                                    <div className='flex gap-2'>
                                        <input className='w-full px-3 py-2 border border-slate-200 outline-0 focus:border-green-500 rounded-sm' 
                                        type='text' 
                                        value={coupon} 
                                        onChange={(e) => setCoupon(e.target.value)} 
                                        placeholder='Input VauChar Coupons'/>
                                        <button onClick={applyCoupon} className='px-5 py-[1px] bg-[#059473] text-white rounded-sm uppercase text-sm'>Apply</button>

                                    </div>
                                    {isCouponValid && <p className='text-green-500'>Coupon Apply: -${discount}</p>}
                                    <div className='flex justify-between items-center'>
                                        <span>Total</span>
                                        <span className='text-lg text-[#059473]'>${(price + shipping_fee) - discount}</span>
                                    </div>
